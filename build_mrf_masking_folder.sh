#!/usr/bin/env bash
# Populate preprocessing_outputs/masking/ with two bundles:
...
if [[ "${SKIP_MRF_PREBUILD:-0}" == "1" ]]; then
  echo "[build_mrf_masking_folder] SKIP_MRF_PREBUILD=1 — exiting without changes."
  exit 0
fi
...
echo "[build_mrf_masking_folder] project: $ROOT"

# --- (1/2) pub_reconstruction ---
echo "[build_mrf_masking_folder] (1/2) pub reconstruction → preprocessing_outputs/masking/pub_reconstruction/"
_pub_hdf5=""
...
if [[ -n "$_pub_hdf5" ]]; then
  echo "[build_mrf_masking_folder] pub: using existing HDF5 (set FORCE_MRF_PYWR_SIM=1 to re-run Pywr): ${_pub_hdf5}"
  python -m methods.preprocessing.build_mrf_active_masks \
    --mask-bundle pub_reconstruction \
  ...
  echo "[build_mrf_masking_folder] pub: no existing HDF5 matched — running Pywr simulation"
  python -m methods.preprocessing.build_mrf_active_masks \
    --mask-bundle pub_reconstruction \
    --inflow-type "$PUB_INFLOW" \
...
if [[ "${SKIP_PERFECT_MRF_PREBUILD:-0}" == "1" ]]; then
  echo "[build_mrf_masking_folder] SKIP_PERFECT_MRF_PREBUILD=1 — done (pub only)."
  exit 0
fi

# --- (2/2) perfect_information (HDF5 only — never Pywr) ---
echo "[build_mrf_masking_folder] (2/2) perfect information → preprocessing_outputs/masking/perfect_information/"
...
if [[ -n "$_perf_hdf5" ]]; then
  echo "[build_mrf_masking_folder] perfect: extracting from HDF5 (Pywr is not run for this bundle): ${_perf_hdf5}"
  python -m methods.preprocessing.build_mrf_active_masks \
    --mask-bundle perfect_information \
    --inflow-type "$PERFECT_INFLOW" \
    --skip-simulation \
    --existing-output "$_perf_hdf5"
else
  echo "ERROR: perfect_information masking needs an existing HDF5; this script never runs Pywr for perfect." >&2
...
echo "[build_mrf_masking_folder] done."
echo "  $ROOT/preprocessing_outputs/masking/pub_reconstruction/lower_basin_mrf_active_ranges.json"
echo "  $ROOT/preprocessing_outputs/masking/perfect_information/lower_basin_mrf_active_ranges.json"
